...
kendallAll <- function(x){
  ...
  df_fits_tot <- trajData %>%
    ...
    do(fits = tidy(kendallTrendTest(count_mean ~ year, ci.slope=FALSE, data=.))) %>% 
    unnest(fits)
...
km_clust_lf <- function(x, lims, threshold){
  ...
  x_data_id <- x %>% 
    filter(data_id == all_of(data_id_temp) &
                          year >= lims[1] &
                          year <= lims[2])
      ...
      for(j in 1:length(ids)){
        ...
        prop_years >= threshold){  
          ...
          df_kendall <- df_clust_lf %>%
              ...
              dplyr::mutate(year = as.integer(year),
                     count = log10(count)) %>%
              do(tidy(suppressWarnings(kendallTrendTest(count ~ year, ci.slope=FALSE, data=.))))
